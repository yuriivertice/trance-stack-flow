# FLOWMIN [![Storybook](https://cdn.jsdelivr.net/gh/storybookjs/brand@main/badge/badge-storybook.svg)](https://yuriivertice.github.io/trance-stack-flow)
...
First, head over to trance-stack-flow/settings/actions and under the `Workflow permissions`
...
Next, head over to trance-stack-flow/settings/branches and add a few branch protection rules.
...
Next, head over to trance-stack-flow/settings/pages and make sure the `Source` is set
to `GitHub Actions`.
...
For now, go to trance-stack-flow/settings/environments and create the following environments:
...
Navigate to trance-stack-flow/actions/workflows/ephemeralDeploy.yml and click the "Run workflow"
button.
...
```bash
 npx cdk deploy flowmin-ephemeral -O /tmp/deployment.result.json \
...
It can be either `flowmin-ephemeral` or `flowmin-production`.
...
[gh-variables]: trance-stack-flow/settings/variables/actions

[gh-secrets]: trance-stack-flow/settings/secrets/actions
